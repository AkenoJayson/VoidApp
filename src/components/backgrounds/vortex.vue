...
function initParticle(i: number) {
    ...
    particleProps.value.set([particleCache.x, particleCache.y, particleCache.vx, particleCache.vy, particleCache.life, particleCache.ttl, particleCache.speed, particleCache.radius, particleCache.hue], i);
}
...
function updateParticle(i: number) {
    ...
    particleCache.x = propsArray[i]!;
    particleCache.y = propsArray[i + 1]!;
    particleCache.vx = propsArray[i + 2]!;
    particleCache.vy = propsArray[i + 3]!;
    particleCache.life = propsArray[i + 4]!;
    particleCache.ttl = propsArray[i + 5]!;
    particleCache.speed = propsArray[i + 6]!;
    particleCache.radius = propsArray[i + 7]!;
    particleCache.hue = propsArray[i + 8]!;

    const n = noise3D(particleCache.x * X_OFF, particleCache.y * Y_OFF, tick.value * Z_OFF) * NOISE_STEPS * TAU;
    ...
    context.strokeStyle = `hsla(${particleCache.hue},100%,60%,${fadeInOut(particleCache.life, particleCache.ttl)})`;
    context.beginPath();
    ...
    propsArray[i + 4] = particleCache.life + 1;

    if (nextX > canvas.width || nextX < 0 || nextY > canvas.height || nextY < 0 || particleCache.life > particleCache.ttl) {
        initParticle(i);
    }
}
